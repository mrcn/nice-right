...
## Approach
- Reuse task .2 Turnstile + ratelimit + `putScan` — do not fork parallel helpers.
...
- Cache via `putScan` with **opaque random `scanId`** (not hash of inputs); content key = normalized URL+name+city for 24h dedupe.
- Expensive-path rate limit (~5/hr/IP) **before** upstreams; **cache hits skip that budget**; 429 includes `Retry-After` from limiter reset.
...
- Shared modules from task .2 (`getScan`/`putScan`, Turnstile, Redis)
...
## Acceptance
- [ ] Implements task .2 `putScan`/`getScan` contract (no competing cache shape)
...
- [ ] Cache hits skip expensive-path rate limit; 429 returns Retry-After
